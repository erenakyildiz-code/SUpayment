import 'dart:ui';
import "dart:convert";
import 'package:blockchainapp2/QRCodeGeneratorPage.dart';
import 'package:blockchainapp2/models/backend.dart';
...
import "package:blockchainapp2/models/APIservices.dart";
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future getProjects() async{
    ...
    returnValue = await apiClient.get(Uri.parse("http://10.0.2.2:5000/Project/Get"));
    print("kanser");
  ...
  Future<List<dynamic>> query(String addr,String functionName,List<dynamic> arguments)async{
  ...
  Future<void> getBalance() async
  {
    ...
    List<dynamic> result = await query(globals.suCoinContract,"balanceOf",[globals.userAddress]); //FONKSIYON ISMI + ARGUMANLAR
    ...
    for(int i = 0; i<globals.tokenAddresses!.length; i++){
      ...
      result = await query(globals.tokenAddresses!.keys.elementAt(i),"balanceOf",[globals.userAddress]);
      nameResult = await query(globals.tokenAddresses!.keys.elementAt(i),"symbol",[]);
...
//creates project obj. list
  ...
  final maestroContract = DeployedContract(ContractAbi.fromJson(ABI,"Maestro"), EthereumAddress.fromHex(contractAddress));
  final ethFunction = maestroContract.function("projectTokens");
  for(int i = 0; i < globals.projectList.length; i++) {
    List<dynamic> addr = [globals.projectList[i],];
  ...
  Future<void> useSwap (BigInt amount, bool whatCoin)async{
    ...
    String contractAddress =  globals.swapContract; //DEPLOYMENT ADDRESS COMES HERE
    final swapContract = DeployedContract(ContractAbi.fromJson(ABI,"Exchange"), EthereumAddress.fromHex(contractAddress));
    List<dynamic> arguments = [];
    arguments.add(amount);
    arguments.add(whatCoin);
    final swapFunction = swapContract.function("Swap");
    //APPROVAL YAZ
    List<dynamic> swapApprovalArguments = [EthereumAddress.fromHex(globals.swapContract),amount];
    setState(() {
      isLoading = true;
      condition = "Approving";
    });
    if(whatCoin){
      final contract = DeployedContract(ContractAbi.fromJson(contractAbi,"HW2Token"), EthereumAddress.fromHex(globals.suCoinContract));
      final approval = contract.function("approve");

      approvalResult = await avaxClient.sendTransaction(globals.credentials, Transaction.callContract(contract: contract, function: approval, parameters: swapApprovalArguments));
    }
    else{
      final contract = DeployedContract(ContractAbi.fromJson(contractAbi,"HW2Token"), EthereumAddress.fromHex(globals.biLiraContract));
      final approval = contract.function("approve");
      approvalResult = await avaxClient.sendTransaction(globals.credentials, Transaction.callContract(contract: contract, function: approval, parameters: swapApprovalArguments));
    ...
    final result = await avaxClient.sendTransaction(globals.credentials, Transaction.callContract(contract: swapContract, function: swapFunction, parameters: arguments));
    print(result);
    setState(() {
      isLoading = false;
    });
  ...
  Future<void> createTransactionList() async
  {
    ...
    globals.ERC20ReturnValue = await json.decode(ERC20ReturnValue.body)["result"];
    ...
    for(int i = 0; i < globals.ERC20ReturnValue.length; i++){
      if(globals.ERC20ReturnValue[i]["contractAddress"] != globals.suCoinContract){
        globals.tokenAddresses![globals.ERC20ReturnValue[i]["contractAddress"]] = globals.ERC20ReturnValue[i];
      }
    }
                          ...
                          children: <Widget>[
                            Flexible(
                                ...
                                child: Text(visible == true
                                    ...
                                    ? "$myData\₺SU"
                                    : '${"$myData\₺SU".replaceAll(RegExp(r"."), "*")}', style: TextStyle(
                ...
                child:
                  ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int indexing) {
                        if(indexing == globals.tokenAddresses!.length){
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              title: Text((
                                  ...
                                  globals.balances[indexing] // token amount

                              ),
                              ),
                              subtitle: Text(
                                  globals.tokenNames[indexing]), // token symbol
                            ),
                          );
                        }
                        else {
                        ...
                        ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int indexing) {
                            ...
                            double gasPriceInAvax = (gas! * gasPrice!) /
                                BigInt.tryParse("1000000000000000000")!;
                            DateTime txTime = DateTime.fromMillisecondsSinceEpoch(
                                int.tryParse(
                                    globals.returnValue[index]["timeStamp"])! *
                                    1000);
                            ...
                            if(globals.returnValue[index]["input"].length > 10){
                              if(globals.returnValue[index]["input"].substring(0,10) == "0x095ea7b3"){
                            ...
                            while (iterator < globals.ERC20ReturnValue.length) {
                              if (globals.returnValue[index]["hash"] == globals.ERC20ReturnValue[iterator]["hash"]
                                  ) {
                                if (globals.ERC20ReturnValue[iterator]["from"] ==
                                    "0x0000000000000000000000000000000000000000") {
                                  SendRecv = 3;
                                }
                                //check if interaction is between user and swap

                                if(globals.ERC20ReturnValue[iterator]["to"] == globals.swapContract){

                                  //check lenght for iterator+1
                                  if(globals.ERC20ReturnValue.length > iterator + 1){
                                    if(globals.ERC20ReturnValue[iterator+1]["from"] == globals.swapContract){
                                      SendRecv = 5;
                                      ERC20Value =  "Sent:" + globals.ERC20ReturnValue[iterator]["value"] + " "+ globals.ERC20ReturnValue[iterator]["tokenSymbol"] + "\nReceived: " + globals.ERC20ReturnValue[iterator+1]["value"]+ " "+globals.ERC20ReturnValue[iterator+1]["tokenSymbol"] ;
                                    }

                                  }

                                }
                                else if(globals.ERC20ReturnValue[iterator]["input"].substring(0,10) == "0x095ea7b3"){
                                  return SizedBox.shrink();
                                  ...
                                  ERC20Value =
                                      globals.ERC20ReturnValue[iterator]["value"] +
                                          " " + globals
                                          .ERC20ReturnValue[iterator]["tokenSymbol"];
                                }
                            ...
                            if (globals.returnValue[index]["isError"] == "1") {
                              return Card(
                                child: ListTile(
                                  leading: Icon(Icons.warning),
                                  title: Text('Block ${globals
                                      .returnValue[index]["blockNumber"]}'),
                                  tileColor: Colors.white,
                              ...
                              if (globals.returnValue[index]["contractAddress"] ==
                                  "") {
                                if (SendRecv == 1)
                                  //sender
                                    {
                                  return Card(
                                    ...
                                    ListTile(
                                      leading: Icon(
                                          CupertinoIcons.arrow_up_circle, size: 30),
                                      title: Text('Sent ${ERC20Value}'),
                                      subtitle: Text('Time: ${txTime
                                          .hour}:${minute} Date: ${txTime
                                          .day}/${txTime.month}/${txTime.year}'),
                                      trailing: Column(
                                          mainAxisAlignment: MainAxisAlignment
                                              .center,
                                          children: <Widget>[

                                            Text('$sentBalance AVAX',
                                              style: TextStyle(fontSize: 15),),
                                            Text('Gas: ${roundDouble(
                                                gasPriceInAvax, 6)}',
                                  ...
                                  return Card(
                                    ...
                                    ListTile(
                                      leading: Icon(
                                          CupertinoIcons.arrow_up_arrow_down_circle, size: 30),
                                  ...
                                  return Card(
                                    ...
                                    ListTile(
                                      leading: Icon(
                                          ...
                                          size: 30),
                                      title: Text('THIS IS A BUG $ERC20Value'),
                                      subtitle: Text('Time: ${txTime
                                          .hour}:$minute Date: ${txTime
                                          .day}/${txTime.month}/${txTime.year}'),
                                      trailing: Column(
                                ...
                                return Card(
                                  child: ListTile(
                                    leading: Icon(Icons.wysiwyg_sharp, size: 30),
                                    tileColor: Colors.white,
                  ...
                  child: Container(
                    ...
                    decoration: InputDecoration(
                      ...
                      enabledBorder: OutlineInputBorder(
                        borderSide: BorderSide(width: 3, color: globals.fourthColor),
        ...
        body: Container(
          ...
          child: Column(
            children: [
              ...
              Center(
                ...
                child: Text("Funding", style: TextStyle(color: globals.secondaryColor,
                fontWeight: FontWeight.w300,
                fontSize: 28),),
              ),
              SizedBox(height: 10,),
              Center(
                child: Text("Fund your favourite projects on Sabanci University", style: TextStyle(color: globals.secondaryColor,
                    fontWeight: FontWeight.w700,
                    fontSize: 16),),
              ),
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemCount: testObjects.length, itemBuilder:
              ...
              if(maestroLoaded) {
                return Card(
                  child: ListTile(
                    ...
                    subtitle: Column(
                      children: [
                        Text("Auction address: " + projectIDs[index].toString()),
                        Text("Token address: "+testIcons[index].toString()),
                      ],
                    ),

                    title: Text("Proposer: "+

                        testObjects[index].toString()
